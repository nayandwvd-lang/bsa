...
function addBadges(md) {
  ...
  const KINDS = {
    '!': ['bsa-mand', 'Mandatory'],
    '?': ['bsa-disc', 'Discretionary'],
    '=': ['bsa-art', 'Term of art']
  }

  md.inline.ruler.before('link', 'bsa_badge', (state, silent) => {
...
export function bsaContainers(md) {
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<div class="bsa-oneminute">' +
        '<p class="bsa-oneminute-head">' +
        '<span class="bsa-oneminute-kicker">In one minute</span>' +
        (title ? `<span class="bsa-oneminute-title">${esc(title)}</span>` : '') +
        '</p>\n'
      )
    },
    () => '</div>\n'
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<div class="bsa-story">' +
        '<p class="bsa-story-head">' +
        '<span class="bsa-story-badge">Worked example · invented facts</span>' +
        (title ? `<span class="bsa-story-title">${esc(title)}</span>` : '') +
        '</p>\n'
      )
    },
    () => '</div>\n'
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<div class="bsa-proviso">' +
        '<p class="bsa-proviso-head">' +
        '<span class="bsa-proviso-kicker">Gating proviso</span>' +
        (title ? `<span class="bsa-proviso-title">${esc(title)}</span>` : '') +
        '</p>\n'
      )
    },
    () => '</div>\n'
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<div class="bsa-instinct">' +
        '<p class="bsa-instinct-head">' +
        '<span class="bsa-instinct-kicker">Test your instinct</span>' +
        (title ? `<span class="bsa-instinct-title">${esc(title)}</span>` : '') +
        '</p>\n'
      )
    },
    () => '</div>\n'
  ...
  addContainer(
    ...
    (tokens, idx) => {
      ...
      return (
        '<details class="bsa-ruling">' +
        `<summary><span>${esc(label)}</span></summary>` +
        '<div class="bsa-ruling-body">\n'
      )
    },
    () => '</div></details>\n'
  )
